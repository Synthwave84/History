...
/*  문제
    ...
    참고 >사원테이블 : EMPLOYEES 프로젝트 내역 : JOB_HISTORY
    질의 시나리오1 : 두 테이블을 참조하여, 사원의 프로젝트정보.(JOB_ID)의 데이터를 조회하라
                     컬럼 = 사원번호, 사원이름 부서코드, 시작날짜, 종료날짜, 프로젝트ID
    INNER JOIN 
    1. 오라클
    2. ANSI

    질의 시나리오 2 : 2테이블을 참조하여, 사원중 프로젝트 정보 데이터가 존재하지 않는 사원도 조회
    OUTER JOIN
    1. 오라클
    2. ANSI
    
    질의 시나리오 3 : 2테이블을 참조하여 사원의 프로젝트정보의 데이터를 조회.
                      사원중 프로젝트 정보 데이터가 존재하지 않는 사원 만 출력.
*/
...
-- 기획부를 상위부서로 하는 하위부서 코드
SELECT  DEPARTMENT_ID 
FROM    DEPARTMENTS 
WHERE   PARENT_ID = 90;

-- 급여 (SALARY) : EMPLOYEES 테이블 
