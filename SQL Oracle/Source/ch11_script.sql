...
-- 묵시적 커서와 커서속성
-- 묵시적커서? SQL 문장(INSERT, UPDATE, MERGE, DELETE, SELECT INTO)이 실행될 때마다 자동으로 만들어져 사용됨.

-- 질의? 사원 테이블에서 80번 부서에 속한 사원은 총 34명인데, 이를 UPDATE 하는 아래의 익명 블록을 실행해 보자.
...
BEGIN
	-- 80번 부서의 사원이름을 자신의 이름으로 갱신
	 UPDATE employees
	   ...
	   WHERE department_id = vn_department_id;	 -- 묵시적 커서 생성.  
	   
	-- 몇 건의 데이터가 갱신됐는지 출력   
	DBMS_OUTPUT.PUT_LINE(SQL%ROWCOUNT); -- 34건
	COMMIT;
...
BEGIN
	...
	-- 2단계) 커서 오픈 (매개변수로 90번 부서를 전달)
	OPEN cur_emp_dep (90); -- 커서호출(실행.)
	...
	LOOP
	  ...
	  EXIT WHEN cur_emp_dep%NOTFOUND; -- 읽어온 데이타가 존재하면 true, 읽어온 데이타가 없으면 false
	  
      
	  -- 사원명을 출력
	  DBMS_OUTPUT.PUT_LINE(vn_count || '번 ' || vs_emp_name); 
      
	
    END LOOP;
  
  -- 커서 닫기
  CLOSE cur_emp_dep;    -- 메모리 소멸. 커서가 가리키는 데이타영역을 닫기.
END;
...
BEGIN
    OPEN cur_emp_dep(90); -- SELECT문의 실행결과가 메모리상 존재하고, 그 영역을 관리하는 이름 cur_emp_dep 커서.
    
    LOOP    -- 커서가 가리키는 영역에 데이타가 3개면, 4번째 동작시 읽어올 데이타(FETCH)가 없어서 루프를 종료한다.
        FETCH cur_emp_dep INTO vs_emp_name;
        
        -- FETCH 작업의 상태를 커서속성 명령어로 체크.
...
BEGIN
	...
	-- FOR문을 통한 커서 패치작업.  emp_rec : 레코드타입의 변수. 즉 데이터행을 가리키는 의미.
	FOR emp_rec IN cur_emp_dep(90) -- OPEN cur_emp_dep(90) 내부적으로 동작된다. FETCH 작업도 진행됨
	LOOP
	  
	  -- 사원명을 출력, 레코드 타입은 레코드명.컬럼명 형태로 사용
	  DBMS_OUTPUT.PUT_LINE(emp_rec.employee_id || ' ' || emp_rec.emp_name);
	
    END LOOP;  -- CLOSE 커서도 생략되어 있음.
  
END;

-- 시나리오 : 부서테이블 참조하여, 부서번호와 부서이름을 커서로 정의하고 데이타를 출력하라.
-- 커서명 : CUR_DEPT_ALL_INFO
DECLARE
   
   -- 커서 선언, 매개변수로 부서코드를 받는다. 커서의 매개변수가 없는 경우 () 가로는 코드에서 제외한다.
...
BEGIN
	...
	LOOP
	  -- 부서번호, 부서이름을 출력, 레코드 타입은 레코드명.컬럼명 형태로 사용
...
-- 커서변수  https://thebook.io/006696/0275/
/*
① 커서 변수 선언하기
커서 변수를 선언하는 방법에는 두 가지가 있는데, 첫 번째는 참조용 커서 타입을 생성하고 나서 이 타입에 대한 커서 변수를 선언하는 방법이다.

    ⓐ TYPE 커서_타입명 IS REF CURSOR [ RETURN 반환 타입 ] ;  -- 커서타입 선언
    ⓑ 커서_변수명 커서_타입명;
*/ 

-- 커서변수를 이용한 작업.   명시적 커서선언작업과는 현재 예제는 결과는 동일하다.
DECLARE
   -- 사원명을 받아오기 위한 변수 선언
   vs_emp_name employees.emp_name%TYPE;
   
   -- 약한 커서타입 선언(사용자정의 데이타타입)
   TYPE emp_dep_curtype IS REF CURSOR; -- RETURN 문이 없으므로 약한 커서타입 이라고 한다.  
   -- 커서변수 선언. SELECT문이 제한이 없다.
   emp_dep_curvar emp_dep_curtype;  -- emp_dep_curvar SYS_REFCURSOR; 동일하다.(기본 내장 데이타타입)
...
-- SYS_REFCURSOR : 오라클이 빌트인으로 제공하는 약한 커서타입
DECLARE
   -- 사원명을 받아오기 위한 변수 선언
   vs_emp_name employees.emp_name%TYPE;
...
DECLARE
   ...
   emp_dep_curvar SYS_REFCURSOR;  --1)커서변수선언
   
    -- 사원명을 받아오기 위한 변수 선언
   vs_emp_name employees.emp_name%TYPE;     --2)일반변수선언
   
   -- (ⅱ) 커서변수를 매개변수르 받는 프로시저, 매개변수는 SYS_REFCURSOR 타입의 IN OUT형
   -- 3)프로시저 선언(정의). 생성
   PROCEDURE test_cursor_argu ( p_curvar IN OUT SYS_REFCURSOR) -- 매개변수가 커서변수로 사용
   IS
...
BEGIN
   -- 프로시저를 호출한다. 
   test_cursor_argu (emp_dep_curvar); -- 커서변수
