...
기존 설계된 틀의 자원과 코드 및 트래픽을 낭비하지 않기 위해 사용된다.
과거 XMLHttpRequest 객체(브라우저 탑재): 를 이용하여 사용했다.


HTML 상태 코드
status :200 ok : true = 서버에서 통신이 잘 되었다. 
status : 201 ok : true = 서버에 문제없이 자료를 잘 전달 하였다.
https://developer.mozilla.org/ko/docs/Web/HTTP/Status/201

FETCH 
현재는 AJAX의 대체제로 FETCH를 사용한다.
윈도우에 지원하는 API로써, AJAX보다 훨씬 사용법이 간편해 졌다.
기능은 동일하나, 사용해야 하는 구문이 훨씬 단순해지므로써, 앞으로 Ajax 기술은
보고 어떠한 동작을 하는지 이해를 하는 정도로 그치면 될 듯 하고, 이후에 만일 작성을 하게 된다면
AJAX보다 Fetch를 이용해야 하지 않을까. 생각한다.
