...
- delete : 데이터 삭제
 ...
 - 현재 : Node.js  덕분에 브라우저를 사용하지않고 ,자바스크립트 코드를 앱처럼 실행을 가능하게       해준다. 결과적으로 서버기반의 백엔드 작업을 지원한다. 

웹 개발자 :
Front-End : 태그를 참조하여, 제어 및 조작하는 기능을 목적. 
Back-End : 서버기반의 기능 구현을 목적. 

변수선언 (정의)
- 기억장소를 생성하고, 기억장소를 가리키는 이름을 만든다는 의미. 


- 엄격모드 : "use strict" 
     자바스크립트 첫줄에 작성, 또는 함수안의 첫 줄에 작성.
     var, let, const 키워드를 이용하여 변수를 선언한다.
     키워드를 이용하지 않고도 바로 변수를 사용.