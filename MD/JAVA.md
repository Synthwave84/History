...
공인 IP
 ...
 DNS 서버 
 - 브라우저 주소표시줄에 www.naver.com입력
 1. 자신의 컴퓨터에 설정된 dnsp 서버에게 naver.com주소로 운영되는 웹사이트
     서버의 ip를 찾아달라는 요청이 진행된다.
  2. DNS서버는 www.naver,com 주소로 운영되는 웹서버의 ip주소를 응답해준다.

TCP 프로토콜 
- 연결지향성. 고정된 회선을 쓴다. 즉 상시연결이되어있다는 느낌.
  먼저 연결을 하고 회선을 고정. 데이터를 순차적으로 보냄. 데이터가 안정성이 있음.
  상대적으로 속도가 느릴 수 있음. 데이터신뢰도가 중요할 경우 TCP
UDP 프로토콜 
